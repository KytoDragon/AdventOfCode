...
void day_1(string input_file, int expected_result_1, int expected_result_2) {
    ...
    while (!fr.end_of_data()) {
        ...
        uint distance;
        if (!parse_integer(line[1..$], &distance) || distance == 0)
        ...
        if (direction == 'R') {
            ...
            if (position > 0 && distance >= position)
                num_zero_hits += 1;
            num_zero_hits += -(position - cast(int)distance) / 100;
    ...
    bool result_1_matched = expected_result_1 == num_zero_position;
    bool result_2_matched = expected_result_2 == num_zero_hits;
    log_result(2025, 1, input_file, "Number of times the dial is left pointing at 0: %#, Number of times the dial hit 0: %#", result_1_matched, num_zero_position, result_2_matched, num_zero_hits);
}
