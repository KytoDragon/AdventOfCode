...
import util.Basic : free, u64, new_array, zero_memory, swap;
import util.Files : read_entire_file;
import util.Strings : index_of, contains;
...
void day_7(string input_file, int expected_result_1, u64 expected_result_2) {
    ...
    string source = fr.read_line();
    int source_index = cast(int)index_of(source, 'S');
    if (source_index < 0)
        return log_error(2025, 7, "Missing beam source in file ", input_file);

    // these arrays contain the number of timelines that reached the given position
    u64[] beam_locations = new_array!u64(source.length);
    u64[] next_beam_locations = new_array!u64(source.length);
    scope(exit) free(beam_locations);
    scope(exit) free(next_beam_locations);
    beam_locations[source_index] = 1;

    int num_beam_split;
    while (!fr.end_of_data()) {
        // every second line is just empty space
        string emtpy_line = fr.read_line();
        if (contains(emtpy_line, '^'))
            return log_error(2025, 7, "Missing empty line in file ", input_file);
        if (fr.end_of_data())
            break;
        
        string splitters = fr.read_line();
        if (splitters.length != source.length)
            return log_error(2025, 7, "Missmatched line length in file ", input_file);
        
        // technically we do not need to check all characters of the line, as the beam can only spread by one column for each line of splitters, but the time saved is minimal.
        foreach (i, char c; splitters) {
            if (beam_locations[i] > 0) {
                if (c == '^') {
                    next_beam_locations[i] = 0;
                    next_beam_locations[i-1] += beam_locations[i];
                    next_beam_locations[i+1] += beam_locations[i];
                    num_beam_split += 1;
                } else {
                    next_beam_locations[i] += beam_locations[i];
                }
            }
        }
        swap(next_beam_locations, beam_locations);
        zero_memory(next_beam_locations);
    }
    
    u64 num_beam_timelines;
    foreach (u64 beam; beam_locations) {
        num_beam_timelines += beam;
    }
